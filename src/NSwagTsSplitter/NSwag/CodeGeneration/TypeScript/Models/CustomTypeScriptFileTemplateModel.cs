...
public class CustomTypeScriptFileTemplateModel
{
    ...
    public CustomTypeScriptFileTemplateModel(
        ...
        TypeScriptTypeResolver resolver)
    {
        ...
        var frameworkModelType = typeof(TypeScriptFrameworkModel);
        var frameworkModelTypeConstructor = frameworkModelType.GetConstructor(BindingFlags.Instance | BindingFlags.NonPublic, new[] { settings.GetType() });
        Framework = frameworkModelTypeConstructor?.Invoke(new object[] { settings }) as TypeScriptFrameworkModel;
    }
